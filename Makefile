...
_test-version:
	...
	@if [ "$(VERSION)" = "latest" ]; then \
		...
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) | grep -E "^checkmake $${LATEST}"; then \
			docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG); \
			echo "Failed"; \
			exit 1; \
		fi; \
	else \
		echo "Testing for tag: $(VERSION)"; \
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) | grep -E "^checkmake $(VERSION)"; then \
			docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG); \
			echo "Failed"; \
			exit 1; \
		fi; \
